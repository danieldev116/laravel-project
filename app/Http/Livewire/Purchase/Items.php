...
class Items extends Component
{
    ...
    public function mount($purchase_id) {
        ...
        // set purchase items in case of edit //
        if(!empty($purchase_id)) {
            ...
            $this->tax_rate = $purchase->tax_rate;
    ...
    public function addItem($product_id)
    {
        ...
        if (!empty($this->items[$product_id])) {
            $this->emitTo('purchase.products-filter', 'errorHappen', 'Product "' . $this->items[$product_id]['name'] . '" already added');
        } else {
    ...
    private function calculateTaxAndDiscount($p) {
            ...
            if (!empty($p['discount_type']) && !empty($p['discount_value'])) {
                $peritem_discount = 0;
                if ($p['discount_type'] == Purchase::FIXED) {
                    $peritem_discount = $p['discount_value'];

                } elseif ($p['discount_type'] == Purchase::PERCENTAGE) {
            ...
            if (!empty($p['tax_type']) && !empty($p['tax_value'])) {
                $peritem_tax = 0;
                if ($p['tax_type'] == Purchase::EXCLUSIVE) {
                    $peritem_tax = ($p['net_unit_cost'] * $p['tax_value']) / 100;
                } else if ($p['tax_type'] == Purchase::INCLUSIVE) {
    ...
    {
        return view('livewire.purchase.items')->with([
