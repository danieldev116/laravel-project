...
class Items extends Component
{
    ...
    public function mount($sale_id) {
        ...
        // set sale items in case of edit //
        if(!empty($purchase_id)) {
            $sale = Sale::query()->findOrFail($sale_id);
            if($sale->saleItems->count() > 0) {
                foreach($sale->saleItems as $pitem) {
                    $p = $pitem->product->prepareAttributes();
                    $p['sale_item_id'] = $pitem->id;
                    $p['isEdit'] = 1;
    ...
    public function addItem($product_id)
    {
        ...
        if (!empty($this->items[$product_id])) {
            $this->emitTo('sale.products-filter', 'errorHappen', 'Product "' . $this->items[$product_id]['name'] . '" already added');
        } else {
    ...
    private function calculateTaxAndDiscount($p) {
            ...
            if (!empty($p['discount_type']) && !empty($p['discount_value'])) {
                $peritem_discount = 0;
                if ($p['discount_type'] == Sale::FIXED) {
                    $peritem_discount = $p['discount_value'];

                } elseif ($p['discount_type'] == Sale::PERCENTAGE) {
            ...
            if (!empty($p['tax_type']) && !empty($p['tax_value'])) {
                $peritem_tax = 0;
                if ($p['tax_type'] == Sale::EXCLUSIVE) {
                    $peritem_tax = ($p['net_unit_cost'] * $p['tax_value']) / 100;
                } else if ($p['tax_type'] == Sale::INCLUSIVE) {
    ...
    {
        return view('livewire.sale.items');
    }
}
