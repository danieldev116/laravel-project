...
class ProductRepository extends BaseRepository
{
    ...
    /**
     * Get brands dropdown option
     * @return mixed
     */
    public function getBrandsData()
    {
        return Brand::toBase()->pluck('name', 'id')->toArray();

    }

    /**
     * Get suppliers dropdown option
     * @return mixed
     */
    public function getSuppliersData()
    {
        return Supplier::toBase()->pluck('name', 'id')->toArray();

    }

    /**
     * Get warehouses dropdown option
     * @return mixed
     */
    public function getWarehousesData()
    {
        return Warehouse::toBase()->pluck('name', 'id')->toArray();

    }

    /**
     * Get base unit dropdown option
     * @return mixed
     */
    public function getBaseUnitsData()
    {
        $result = BaseUnit::query()->orderBy('name')->get();
        $data = [
            'base_units' => [],
            'base_unit_children' => []
        ];
        foreach ($result as $row) {
            $data['base_units'][$row->id] = $row->name;
            if (empty($data['base_unit_children'][$row->id])) {
                $data['base_unit_children'][$row->id] = [];
            }
            if ($row->units->count() > 0) {
                foreach ($row->units as $ch_row) {
                    $data['base_unit_children'][$row->id][$ch_row->id] = $ch_row->name;
                }
            }
    ...
    public function store($input)
    {
        try {
            DB::beginTransaction();
            $input['warehouse_id'] = $input['purchase_warehouse_id'];
            ...
            $purchaseStock = [
                ...
                'date' => $input['purchase_date'] ?? Carbon::now(),
            ];
