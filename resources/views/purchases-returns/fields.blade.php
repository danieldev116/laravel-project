...
<div class="row">
    <div
        ...
        {{ Form::text('date', $purchase_return->purchase_date ?? null, ['class' => 'form-control', 'id' => 'purchase_return_date', 'autocomplete' => 'off', 'required']) }}
    </div>
    <div class="col-md-4 mb-3">
        {{ Form::label('warehouse_id', __('messages.purchases.warehouse_purchase') . ':', ['class' => 'form-label required mb-3']) }}
        {{ Form::select('warehouse_id', $warehouses, $purchase_return->warehouse_id, ['class' => 'form-select io-select2', 'disabled'=>!empty($warehouse_id), 'onchange'=>'livewire.emitTo("purchase-return.products-filter","loadWarehouseProduct", this.value)', 'id' => 'warehouse_id', 'placeholder' => __('messages.purchases.purchase_choose_warehouse'), 'required', 'data-control' => 'select2']) }}
    ...
    <div class="col-md-4 mb-3">
        ...
        {{ Form::select('supplier_id', $suppliers, $purchase_return->supplier_id, ['class' => 'form-select io-select2', 'id' => 'supplier_id', 'placeholder' => __('messages.purchases.purchase_choose_supplier'), 'required', 'data-control' => 'select2']) }}
    </div>
    
    <livewire:purchase-return.products-filter></livewire:purchase-return.products-filter>

    <livewire:purchase-return.items :purchase_return_id="$purchase_return_id"></livewire:purchase-return.items>
...
<div class="float-end">
    <div class="form-group col-sm-12">
        <button type="button" name="submit" class="btn btn-primary mx-3" id="savePurchaseReturnForm" data-status="0"
...
<script type="text/javascript">
    $(function(e) {
        @if(!empty($warehouse_id))
        livewire.emitTo("purchase-return.products-filter", "loadWarehouseProduct", {{ $warehouse_id }});
        @endif
