<div>
    <div class="col-lg-12 md-12 mt-3"><label class="form-label">Order items:</label><span class="required "></span>
        <div class="table-responsive">
            <table class="table" id="purchase-returns-items">
                <thead>
                <tr>
                        ...
                        <tr>
                            <td>
                                ...
                                {{$item['name']}}
                                    ...
                                    <span class="badge bg-light-primary p-1 ms-1" style="cursor: pointer"
                                          onclick="showPurchaseReturnProductModal({{ $pid }})">
                                ...
                                <div class="custom-qty">
                                    <div class="flex-nowrap input-group">
                                        <span
                                            ...
                                            onclick="livewire.emitTo('purchase-return.items','qtyUpdate','-', '{{$pid}}')">-</span>
                                        <input aria-label="Product Quantity" step="1" min="0" type="text"
                                               ...
                                               onkeyup="returnQtyUpdate(this);"
                                               data-pid="{{$pid}}"
                                               name="purchase_return_items[{{ $i }}][quantity]">
                                        <span
                                            class="btn btn-primary btn-sm px-4 px-4 pt-2 input-group-text"
                                            onclick="livewire.emitTo('purchase-return.items','qtyUpdate','+', '{{$pid}}')">+</span>
                                    </div>
                                    <input type="hidden" name="purchase_return_items[{{ $i }}][product_id]" value="{{ $pid }}" />
                                    <input type="hidden" name="purchase_return_items[{{ $i }}][purchase_return_item_id]" value="{{ $item['purchase_return_item_id'] }}" />
                                    <input type="hidden" name="purchase_return_items[{{ $i }}][isEdit]" value="{{ $item['isEdit'] }}" />
                                    <input type="hidden" name="purchase_return_items[{{ $i }}][product_cost]" value="{{ $item['product_cost'] }}" />
                                    <input type="hidden" name="purchase_return_items[{{ $i }}][tax_type]" value="{{ $item['tax_type'] }}" />
                                    <input type="hidden" name="purchase_return_items[{{ $i }}][tax_value]" value="{{ $item['tax_value'] }}" />
                                    <input type="hidden" name="purchase_return_items[{{ $i }}][discount_type]" value="{{ $item['discount_type'] }}" />
                                    <input type="hidden" name="purchase_return_items[{{ $i }}][discount_value]" value="{{ $item['discount_value'] }}" />
                                    <input type="hidden" name="purchase_return_items[{{ $i }}][purchase_unit]" value="{{ $item['purchase_unit'] }}" />
                                </div>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                <a href="javascript: void(0) " class="text-danger"
                                   onclick="livewire.emitTo('purchase-return','removeItem',{{$pid}})">
    ...
    <input type="hidden" id="purchase_return_items" value="{{ json_encode($items) }}">
</div>
